...
const createTimeline = (title, desc, tags, notes, tagFilter) => {
  ...
  const decades = new Set(datedNotes.map(note => note.getDecade()));
...
const main = async () => {
  ...
  if (config.timelines) {
    config.timelines.forEach(tag => {
      ...
      // WHAT DO WE NEED TO DO FOR EACH 'TIMELINE'
      // create report
      // - with decade breakouts
      // - and with 'undated' section at bottom/top
      const timelineTitle = `${tag} Timeline`;
      createTimeline(timelineTitle, 'A timeline\'s description.', tags, notes, tag);
    });
  }
};
