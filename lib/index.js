...
const addTimedMeta = (report, note, root) => {
  if (note.sources) {
    ...
    note.sources.forEach(source => {
      report.addLine(`- Source: ${source.getStr()}`);
    });
  }
  if (note.citations) {
...
const createReport = (title, desc, notes, tagFilter) => {
...
const main = async () => {
  ...
  createReport(config.title, config.description, notes, null);

  createSummary(tags, timelines);

  tags.forEach(tag => {
    createReport(tag, null, notes, tag);
  });
